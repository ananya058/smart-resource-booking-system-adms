...
const slideshowCarousel = document.getElementById("slideshowCarousel");
...
function loadDefaultSlideshow() {
    // Placeholder slideshow with gradient backgrounds
    const slides = [
        { title: "Welcome to FSET", color: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)" },
        { title: "Smart Room Booking", color: "linear-gradient(135deg, #f093fb 0%, #f5576c 100%)" },
        { title: "Efficient Resource Management", color: "linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)" }
    ];
    populateSlideshow(slides);
}

function populateSlideshow(images) {
    const carouselInner = slideshowCarousel.querySelector('.carousel-inner');
    ...
    images.forEach((image, index) => {
        ...
        if (image.path) {
            // Image file path
            item.innerHTML = `<img src="${image.path}" alt="Slide ${index + 1}">`;
        } else if (image.color) {
            // Gradient background
            item.style.backgroundImage = image.color;
            item.innerHTML = `<div style="width: 100%; height: 100%; display: flex; align-items: center; justify-content: center;">
                <h2 style="color: white; text-shadow: 2px 2px 8px rgba(0,0,0,0.5);">${image.title || ''}</h2>
            </div>`;
        }
...
function loadDefaultGallery() {
    ...
    const placeholders = [
        { icon: '📸', text: 'Campus View' },
        { icon: '🏫', text: 'Classroom' },
        { icon: '🔬', text: 'Laboratory' },
        { icon: '👥', text: 'Faculty' },
        { icon: '📚', text: 'Library' },
        { icon: '🎓', text: 'Events' }
    ];
    populateGallery(placeholders);
}
...
function populateGallery(images) {
    ...
    images.forEach((image, index) => {
        ...
        if (image.path) {
            // Image file path
            const imageUrl = encodeURI(image.path);
            item.innerHTML = `<img src="${imageUrl}" alt="Gallery ${index + 1}">`;
            item.addEventListener('click', () => openImageModal(imageUrl, `Gallery ${index + 1}`));
        } else if (image.icon) {
            // Placeholder with icon
            item.innerHTML = `<div class="gallery-placeholder">${image.icon}</div>`;
        }
...
function displayAboutImage(images) {
    if (!images || images.length === 0) {
        aboutImage.src = '/Gallery/WhatsApp%20Image%202026-05-12%20at%204.02.27%20PM.jpeg';
        aboutImage.alt = 'USTC Building';
        return;
    }

    const preferredName = 'WhatsApp Image 2026-05-12 at 4.02.27 PM.jpeg';
    const preferredImage = images.find(img => img.name === preferredName);
    const image = preferredImage || (images.length > 1 ? images[1] : images[0]);

    if (image && image.path) {
        const imageUrl = encodeURI(image.path);
        aboutImage.src = imageUrl;
        aboutImage.alt = image.name || 'USTC image';
        aboutImage.onclick = () => openImageModal(imageUrl, image.name || 'USTC image');
    } else {
        aboutImage.src = encodeURI('/Gallery/WhatsApp Image 2026-05-12 at 4.02.27 PM.jpeg');
        aboutImage.alt = 'USTC Building';
    }
}
...
function showHome() {
    ...
    // Restart slideshow carousel
    const carousel = new bootstrap.Carousel(slideshowCarousel, {
        interval: 5000,
        wrap: true
    });
}
...
// Add this helper to script.js
function navigateTo(sectionId) {
    // List all main sections
    const sections = [homeSection, aboutSection, roomBooking, departmentSection, contactSection];
...
// Then update your nav calls:
function showContact() { navigateTo("contactSection"); }
function showHome() { navigateTo("home"); }
// etc...